class DBI
	def initialize(uri, opts = {})
		...
		case opts[:dbms]
		when 'mysql'
			opts[:port] = 3306 if (opts[:port].nil? && opts[:host] != 'localhost')
			opts[:encoding] = 'utf8' if opts[:encoding].nil?
		...
		when 'postgres'
			opts[:port] = 5432 if opts[:port].nil?
			opts[:encoding] = 'utf-8' if opts[:encoding].nil?
